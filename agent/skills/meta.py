...
def generate_meta(
    ...
    Returns:
        标题 + 标签 + 封面文案
    """
    quote_standards = "\n".join(f"- {s}" for s in QUOTE_STANDARDS["必备要素"])

    prompt = f"""请为以下内容生成发布配套信息。你是小八，你的标题和封面文案必须符合你的文字标准：有钩子、有画面感、不鸡汤、让人想点进去。
...
只用这3种开头方式：
1. 反常识判断："很多人觉得……但其实……"
2. 具体画面+情绪："那天她站在雨里，突然懂了……"
3. 抛出没说出口的感受："你有没有过一种感觉……"
...
### 标题（{count}个备选）
每个标题用不同风格，但必须符合上述钩子标准：
- 1个「悬念型」（留问号/省略号，引发好奇，3秒内有钩子）
- 1个「共鸣型」（直击情绪，让人想点进去，有画面感）
- 1个「反转型」（打破认知，制造冲突感，不说教）
- 禁止："今天想聊聊""大家好""家人们"
